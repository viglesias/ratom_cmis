...
module Atom
  ...
  module Xml # :nodoc:
   class NamespaceMap
      ...
      def get(ns)	
	if ns == Atom::CMIS_CORE
	  @map[ns] = "ns"
	elsif ns == Atom::CMIS_REST
	  @map[ns] = "cra"
	elsif ns == Atom::CMIS_MESSAGING
	  @map[ns] = "cm"
        elsif ns == Atom::NAMESPACE
          @map[ns] = "atom"
    ...
    module Parseable # :nodoc:
      ...
      def to_xml(nodeonly = false, root_name = self.class.name.demodulize.downcase, namespace = nil, namespace_map = nil)
        namespace_map = NamespaceMap.new(self.class.namespace) if namespace_map.nil?|| self.class.respond_to?(:namespace)
	prefix =  NamespaceMap.new(self.class.namespace).get(self.class.namespace)        	
	node = XML::Node.new(prefix+":"+root_name)
